...
struct bone_transform {
	bone_transform() : position(vec3()), rotation(vec3()) {}
...
public:
	keyframe_s() {}
	keyframe_s(keyframe_s const &kf) { bones_positions = kf.bones_positions; }
	~keyframe_s() {}
...
const int num_bones = CNT(object_hierarchy);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
  ...
  //SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_HIGHEST);
	...
	sort_meshes_by_hierarchy(scene_model, object_hierarchy, num_bones);
	DBG("---------- POST MESHES ------------\n");
	print_meshes(scene_model);

	compute_positions_of_parents(scene_model, object_hierarchy, num_bones);
	if (!prepare_childs_indices(scene_model, object_hierarchy, num_bones)) {
		return 1;
	}
...
void keyframe_grab(keyframe_s &kf, model &mdl)
{
	...
	kf.bones_positions.reserve(num_bones);
	for (size_t i = 0; i < num_bones; i++) {
...
enum ANIM_LDR_STATUS {
	ALDR_OK = 0,
	ALDR_ERROR_OPENING_FILE,
	ALDR_ERROR_ANIM_MISTMATCH
};

const char *p_anim_ldr_errors[] = {
	"No errors",
	"Не удалось создать или открыть файл",
	"Файл анимации повержден или не соответствует требуемому скелету"
};

#define CSTRWL(str) str, sizeof(str) - 1

/* PARSER INFO FLAGS */
#define PIF_NONE 0
#define PIF_VERSION (1 << 0)
#define PIF_FRAMERATE (1 << 1)
#define PIF_NUMFRAMES (1 << 2)
#define PIF_NUMBONES (1 << 3)

inline bool is_all_head_flags(int flags)
{
	return (flags & PIF_VERSION) && (flags & PIF_FRAMERATE) && (flags & PIF_NUMFRAMES) && (flags & PIF_NUMBONES);
}

ANIM_LDR_STATUS animation_import(const char *p_path)
{
	union {
		int parent_id;
		int frame_idx;
	...
	int parser_flags = PIF_NONE;
	int curr_bone = 0;
	int bones_count;
	int frame_rate;
	int num_frames;

	char buffer[512], bonename[64];
	...
	FILE *fp = NULL;
	fopen_s(&fp, p_path, "rt");
	if (!fp)
		return ALDR_ERROR_OPENING_FILE;

	/* FIND HEADER DATA */
	while (!feof(fp) && !ferror(fp) && !is_all_head_flags(parser_flags)) {
		fgets(buffer, sizeof(buffer), fp);
		if (buffer[0] == '/' || buffer[0] == '\n')
			continue;

		if (!strncmp(buffer, CSTRWL("version")) && sscanf(buffer, "version %d", &version) == 1) {
			parser_flags |= PIF_VERSION;
			if (version != ANIM_VERSION) {
				fclose(fp);
				return ALDR_ERROR_ANIM_MISTMATCH;
			}
		}
		else if (!strncmp(buffer, CSTRWL("framerate")) && sscanf(buffer, "framerate %d", &frame_rate) == 1) {
			parser_flags |= PIF_FRAMERATE;
			anim_time_interval = 1.0 / frame_rate;
		}
		else if (!strncmp(buffer, CSTRWL("num_frames")) && sscanf(buffer, "num_frames %d", &num_frames) == 1) {
			parser_flags |= PIF_NUMFRAMES;
		}
		else if (!strncmp(buffer, CSTRWL("num_bones")) && sscanf(buffer, "num_bones %d", &bones_count) == 1) {
			parser_flags |= PIF_NUMBONES;
		}
	}

	if (!is_all_head_flags(parser_flags)) {
		fclose(fp);
		printf("if (!is_all_head_flags(parser_flags)) {");
		return ALDR_ERROR_ANIM_MISTMATCH; //not found needed info
	}
...
/*		if (!strncmp(buffer, CSTRWL("hierarchy"))) {
			do {
				fgets(buffer, sizeof(buffer), fp);
				if (buffer[0] == '/' || buffer[0] == '\n')
					continue;

				if (sscanf(buffer, " %s %d", bonename, &parent_id) != 2) {
					fclose(fp);
					return ALDR_ERROR_ANIM_MISTMATCH;
				}

				if (strcmp(object_hierarchy[curr_bone].p_name, bonename) != 0 || object_hierarchy[curr_bone].parent != parent_id) {
					fclose(fp);
					return ALDR_ERROR_ANIM_MISTMATCH;
				}
				curr_bone++;
			} while (!feof(fp) && !ferror(fp) && buffer[0] != '}' && curr_bone < bones_count);
		}
		else */if (!strncmp(buffer, CSTRWL("frame")) && sscanf(buffer, "frame %d {", &frame_idx) == 1) {
			...
			//if (keyframe.bones_positions.size() != bones_count) {
			//	DBG("frames block corrupted");
			//	recorded_keyframes.clear();
			//	fclose(fp);
			//	printf("if (keyframe.bones_positions.size() != bones_count) {");
			//	return ALDR_ERROR_ANIM_MISTMATCH;
			//}

			recorded_keyframes.push_back(keyframe);
			printf("FRAME %d\n", recorded_keyframes.size());
			printf("keyframe %d\n", keyframe.bones_positions.size());
		}
	}

	for (size_t i = 0; i < recorded_keyframes.size(); i++)
	{
		printf("KF %d {\n", i);
		for (size_t j = 0; j < recorded_keyframes[i].bones_positions.size(); j++)
		{
			printf(" bone %d\n", j);
		}
		printf("}\n\n");
...
ANIM_LDR_STATUS animation_export(const char *p_path)
{
	int i;
	int frame_rate;
	FILE *fp = NULL;
	fopen_s(&fp, p_path, "wt");
	if (!fp)
		return ALDR_ERROR_OPENING_FILE;

	fprintf(fp, "version %d\n", ANIM_VERSION);
	frame_rate = anim_fps_switch.get_pos();
	fprintf(fp, "framerate %d\n", ABS(frame_rate));
	fprintf(fp, "num_bones %d\n", num_bones);
	fprintf(fp, "num_frames %zd\n\n", recorded_keyframes.size());

	/* write hierarchy */
	fputs("hierarchy {\n", fp);
	for (i = 0; i < num_bones; i++)
		fprintf(fp, "\t%s %d // parent %s\n", object_hierarchy[i].p_name, object_hierarchy[i].parent, (object_hierarchy[i].parent == -1) ? "none" : object_hierarchy[object_hierarchy[i].parent].p_name);
	...
	fputs(
		"// Animation keyframes\n"
		"// frame (frame_num) - keyframe sequence\n"
		"// ( pos.x pos.y pos.z ) ( rot.x rot.y rot.z )\n", fp);

	for (i = 0; i < (int)recorded_keyframes.size(); i++) {
		fprintf(fp, "frame %d {\n", i);
		for (size_t j = 0; j < num_bones; j++) {
			bone_transform &transform = recorded_keyframes[i].bones_positions[j];
...
LRESULT CALLBACK controlpanel_wnd_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDC_EXPORT_ANIM:
			strcpy_s(buffer, sizeof(buffer), "untitled.anm");
			GetCurrentDirectoryA(sizeof(current_dir), current_dir);
			status = ctls::dialog_save_file(buffer, sizeof(buffer), HWND_DESKTOP, current_dir, ".ANM file (*.anm)\0*.txt\0All Files (*.*)\0*.*\0");
			if (status == ctls::FD_STATUS_ERROR) {
				MessageBoxA(0, "Произошла ошибка при выборе пути", "GetSaveFileName error", MB_ICONERROR | MB_OK);
				break;
			}
			
			status = animation_export(buffer);
			if (status != ALDR_OK)
				MessageBoxA(HWND_DESKTOP, p_anim_ldr_errors[status], "Export error", MB_OK | MB_ICONERROR);
		...
		case IDC_ANIM_PLAY:
			if (recorded_keyframes.size() > 2) {
				...
				// disable edit keyframes buttons
				EnableWindow(GetDlgItem(hWnd, IDC_KEYFRAME_SET), FALSE);
				EnableWindow(GetDlgItem(hWnd, IDC_KEYFRAME_UNDO), FALSE);
				EnableWindow(GetDlgItem(hWnd, IDC_KEYFRAMES_CLEAR), FALSE);
				EnableWindow(GetDlgItem(hWnd, IDC_KEYFRAME_SET_AS_ENDFRAME), FALSE);
			}
			break;
		...
		case IDC_ANIM_STOP:
			...
			animation_flags &= ~AF_ANIMATE;
			// disable edit keyframes buttons
			EnableWindow(GetDlgItem(hWnd, IDC_KEYFRAME_SET), TRUE);
			EnableWindow(GetDlgItem(hWnd, IDC_KEYFRAME_UNDO), TRUE);
