...
namespace lab12i
{
    ...
    public class MyBSTBuilder
    {
        ...
        public class NodesWalkPath
        {
            ...
            private MyBinaryTreeNode<Republic>?       endNodeRef;
            private NodeSearchContext                seatrchData;
            public  List<MyBinaryTreeNode<Republic>?> ?skipRefs;

            public MyBinaryTreeNode<Republic>? EndNodeRef {
            ...
            private bool NodeInSkipList(MyBinaryTreeNode<Republic>? node)
            {
            ...
            private bool SearchRecursive(MyBinaryTreeNode<Republic>? node) {
            ...
            public bool Search(MyBinaryTreeNode<Republic>? rootNode, List<MyBinaryTreeNode<Republic>?> ?skipList) {
                // recursive search in left branch and in right branch
                skipRefs = skipList; 
        ...
        public readonly List<MyBinaryTreeNode<Republic>?>  searchTreeRootNodes;

        private bool SearchAllPaths(MyBinaryTreeNode<Republic>? rootNode, NodeSearchContext search)
        {
            List<MyBinaryTreeNode<Republic>?> skipNodesList = new List<MyBinaryTreeNode<Republic>?>();
        ...
        private bool SearchPathWithIgnoreExistsRefs(MyBinaryTreeNode<Republic>? rootNode, List<MyBinaryTreeNode<Republic>?> ? skipNodesList)
        {
            NodesWalkPath firstWalk = new NodesWalkPath(search);
            if (firstWalk.Search(rootNode, skipNodesList)) {
                //There are links in the sheet but the function returned false (not found)
        ...
        private bool SearchAllPaths2(MyBinaryTreeNode<Republic>? rootNode, NodeSearchContext search)
        {
            List<MyBinaryTreeNode<Republic>?> skipNodesList = new List<MyBinaryTreeNode<Republic>?>();
        ...
        public void BuildSearchTrees() {
            ...
            for (int i = 0; i < walkPathes.Count; i++) {
                ...
                MyBinaryTreeNode<Republic>? endSrcNode = walkPath.EndNodeRef;
                ...
                MyBinaryTreeNode<Republic>? mySrcRoot = null;
                MyBinaryTreeNode<Republic>? newNode = null;
                MyBinaryTreeNode<Republic>? previousRootNode = null;

                if(endSrcNode != null) {
                    newNode = new MyBinaryTreeNode<Republic>(endSrcNode.data);
                    while (endSrcNode != null) {
                        ...
                        if (mySrcRoot != null) {
                            ...
                            MyBinaryTreeNode<Republic>? myNewRoot = new MyBinaryTreeNode<Republic>(mySrcRoot.data);
        ...
        public MyBSTBuilder(MyBinaryTreeNode<Republic>? rootNode, char firstSym, int charIdx = 0) {
            search = new NodeSearchContext(firstSym, charIdx);
            walkPathes = new List<NodesWalkPath?>();
            searchTreeRootNodes = new List<MyBinaryTreeNode<Republic>?>();
