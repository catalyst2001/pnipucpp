...
void paint_scene(HDC hdc, HWND hwnd)
{
	...
	for (int i = 0; i < num_lines; i++) {
		v0 = project_vertex(house[i][0]);
		v1 = project_vertex(house[i][1]);
	...
	distortion_factor.x = glm::length(glm::vec2(house[2][0].x, house[2][0].y)) / glm::length(glm::vec2(projected_verts[2][0].x, projected_verts[2][0].y));
	distortion_factor.y = glm::length(glm::vec2(house[16][0].x, house[16][0].y)) / glm::length(glm::vec2(projected_verts[16][0].x, projected_verts[16][0].y));
	distortion_factor.z = glm::length(glm::vec2(house[0][0].x, house[0][0].y)) / glm::length(glm::vec2(projected_verts[0][0].x, projected_verts[0][0].y));

	text_print(hdc, 10, 10, "vanishing points count: %d", vanishing_points);
	text_print(hdc, 10, 30, "scene distortion factor: %f %f %f", distortion_factor.x, distortion_factor.y, distortion_factor.z);
	text_print(hdc, 10, 50, "distance: %f", dist);
