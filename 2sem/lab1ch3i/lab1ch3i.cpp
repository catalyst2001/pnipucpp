...
inline node *new_node(node *p_parent, char value)
{
...
{
	const static char alph[] = "QWERTYUIOPASDFGHJKLZXCVBNM";
	return alph[rand() % (sizeof(alph) - 1)];
}
...
node *build_tree()
{
	node *p_root_node = new_node(nullptr, get_random_symbol());
	p_root_node->set_level(1);

	node *p_branch1 = new_node(p_root_node, get_random_symbol());
	node *p_sub1branch1 = new_node(p_branch1, get_random_symbol());
	p_sub1branch1->set_left_node(new_node(p_sub1branch1, get_random_symbol()));
	p_sub1branch1->set_right_node(new_node(p_sub1branch1, get_random_symbol()));
	p_branch1->set_left_node(p_sub1branch1);

	node *p_sub2branch1 = new_node(p_branch1, get_random_symbol());
	p_sub2branch1->set_left_node(new_node(p_sub2branch1, get_random_symbol()));
	p_sub2branch1->set_right_node(new_node(p_sub2branch1, get_random_symbol()));
	p_branch1->set_right_node(p_sub2branch1);
	p_root_node->set_left_node(p_branch1);

	node *p_branch2 = new_node(p_root_node, get_random_symbol());
	node *p_sub1branch2 = new_node(p_branch2, get_random_symbol());
	p_sub1branch2->set_left_node(new_node(p_sub1branch2, get_random_symbol()));
	p_sub1branch2->set_right_node(new_node(p_sub1branch2, get_random_symbol()));
	p_branch2->set_left_node(p_sub1branch2);

	node *p_sub2branch2 = new_node(p_branch2, get_random_symbol());
	p_sub2branch2->set_left_node(new_node(p_sub2branch2, get_random_symbol()));
	p_sub2branch2->set_right_node(new_node(p_sub2branch2, get_random_symbol()));
...
void draw_node(node *p_node, bool dobuleDraw = false)
{
	...
	draw_string_centred(GLUT_BITMAP_TIMES_ROMAN_24, p_node->pos.x - 10, p_node->pos.y, "%c", p_node->get_value());
	glPopAttrib();
...
void draw_scene(void)
{
	...
	glColor3ub(0, 0, 0);
	draw_string(GLUT_BITMAP_TIMES_ROMAN_24, 10, height - 20, "Tree height: %d", tree_depth);
	glPopMatrix();
	glMatrixMode(GL_MODELVIEW);
	glPopMatrix();
	glutSwapBuffers();
...
int main(int argc, char **argv)
{
	...
	process_tree_node(p_root_node, width, ROOT_NODE);
	tree_depth = tree_height(p_root_node);
